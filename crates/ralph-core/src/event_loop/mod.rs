...
impl EventLoop {
    ...
    pub fn check_completion_event(&mut self) -> Option<TerminationReason> {
        ...
        if let Some(reason) = self.judge_rejects_completion() {
            self.bus.publish(Event::new(
                "task.resume",
                format!(
                    "Completion rejected by judge: {reason}. Address the gap and verify it before emitting the completion promise again."
                ),
            ));
            return None;
        }
    ...
    fn judge_rejects_completion(&mut self) -> Option<String> {
        use crate::completion_judge::{JudgeDecision, JudgeRequest, run_judge};

        let judge = self.config.event_loop.completion_judge.clone()?;
        ...
        let closed_tasks = self.closed_task_titles();
        let workspace = self.config.core.workspace_root.clone();
        let request = JudgeRequest {
            schema_version: 1,
            objective: self.ralph.objective(),
            completion_promise: &self.config.event_loop.completion_promise,
            iteration: self.state.iteration,
            rejections: self.state.judge_rejections,
        ...
        let reason = match run_judge(&judge, &workspace, &request) {
            Ok(verdict) if verdict.verdict == JudgeDecision::Pass => {
                info!(reason = %verdict.reason, "Completion judge passed");
                return None;
            }
            Ok(verdict) => verdict.reason,
            Err(error) if judge.fail_closed => format!("judge unavailable ({error})"),
            Err(error) => {
                warn!(%error, "Completion judge failed - accepting completion (fail-open)");
                return None;
            }
        };

        self.state.judge_rejections += 1;
        warn!(
            rejections = self.state.judge_rejections,
            %reason,
            "Rejecting completion event: judge verdict fail"
        );
        Some(if reason.is_empty() {
            "objective not verified".to_string()
        } else {
            reason
        })
    }

    fn closed_task_titles(&self) -> Vec<String> {
        use crate::task_store::TaskStore;
