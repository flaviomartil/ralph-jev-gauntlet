...
#[cfg(unix)]
#[test]
fn test_completion_judge_rejects_then_accepts() {
    let temp_dir = tempfile::TempDir::new().unwrap();
    let (mut event_loop, events_path) = judge_loop(
        &temp_dir,
        "if [ -f passed ]; then echo '{\"verdict\":\"pass\"}'; else touch passed; echo '{\"verdict\":\"fail\",\"reason\":\"tests not run\"}'; fi",
        3,
    );
    ...
    assert_eq!(event_loop.state.judge_rejections, 1);
...
#[cfg(unix)]
#[test]
fn test_completion_judge_error_fails_open() {
    let temp_dir = tempfile::TempDir::new().unwrap();
    let (mut event_loop, events_path) = judge_loop(&temp_dir, "exit 1", 3);
